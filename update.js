...
['native.rb', 'pathname.rb', 'set.rb', 'stringio.rb', 'strscan.rb'].forEach((file) => {
  console.log(`Copy ${opalDirectory}/stdlib/${file} to src/stdlib/${file}`);
  fs.createReadStream(`${opalDirectory}/stdlib/${file}`)
    .pipe(fs.createWriteStream(`src/stdlib/${file}`))
});
